...
struct ProspectAppointmentsView: View {
    ...
    private var selectedAppointments: [Appointment] {
        prospect.appointments.filter { selectedAppointmentIDs.contains($0.id) }
    }
    ...
    private var header: some View {
        HStack(alignment: .center, spacing: 12) {
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text("\(filteredAppointments.count) \(filter.rawValue.lowercased()) for \(prospect.fullName)")
                    .font(.subheadline)
    ...
    private var emptyState: some View {
        VStack(spacing: 10) {
            ...
            Text("No \(filter.rawValue.lowercased()) appointments")
                .font(.headline)
            Text("New meetings for this prospect will appear here.")
                .font(.subheadline)
                .foregroundStyle(.secondary)
    ...
    private func appointmentRow(_ appointment: Appointment) -> some View {
        ...
        return HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                Text(appointment.title.isEmpty ? "Follow Up With \(prospect.fullName)" : appointment.title)
                    .font(.subheadline.weight(.semibold))
                    .foregroundStyle(.primary)
                    .lineLimit(1)

                Label(prospect.address, systemImage: "mappin.and.ellipse")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    .lineLimit(1)
    ...
    private func handleAppointmentTap(_ appointment: Appointment) {
        ...
        if isSelecting {
            ...
            controller.selectedAppointmentDetails = appointment
        }
    }
