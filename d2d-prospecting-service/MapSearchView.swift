...
struct MapSearchView: View {
    ...
    var body: some View {
                ...
                MapDisplayView(
                    ...
                    onMarkerTapped: { place in
                        ...
                        if let mapView = MapDisplayView.cachedMapView {
                            ...
                            // Clamp popup so it never goes offscreen
                            let popupW: CGFloat = 240
                            let popupH: CGFloat = 120
                            let halfW = popupW / 2
                            let halfH = popupH / 2
                            let offsetY = halfH + 14
                            let x = min(max(raw.x, halfW), geo.size.width - halfW)
                            let rawY = raw.y - offsetY
                            let y = min(max(rawY, halfH), geo.size.height - halfH)
        ...
        } message: { Text("Would you like to schedule a follow-up for \(followUpProspectName)?") }
                ...
                ForEach(searchVM.results.prefix(3), id: \ .self) { result in
                    Button {
                        ...
                        VStack(alignment: .leading, spacing: 4) {
                            Text(result.title).font(.body).bold().lineLimit(1).truncationMode(.tail)
                            Text(result.subtitle).font(.subheadline).foregroundColor(.gray).lineLimit(1).truncationMode(.tail)
                        }
                        .padding(.vertical, 10)
                        .padding(.horizontal)
    ...
    private func handleCompletionTap(_ result: MKLocalSearchCompletion) {
        ...
        MKLocalSearch(request: request).start { response, error in
            ...
            DispatchQueue.main.async {
                ...
                controller.region = MKCoordinateRegion(center: mapItem.placemark.coordinate,
                                                       latitudinalMeters: 1609.34,
                                                       longitudinalMeters: 1609.34)
                searchVM.results = []
